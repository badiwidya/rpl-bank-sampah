<div x-data="{ openModal: false }" class="flex justify-center bg-gradient-to-b from-emerald-50 to-emerald-200 min-h-full min-w-full">
    <div class="py-6 sm:py-8 w-full sm:w-11/12 md:w-10/12 lg:w-4/5 mx-auto px-4 sm:px-6">
        ...
        <div class="flex flex-col md:flex-row md:justify-between md:items-center gap-4 mb-6">
            ...
            <div class="flex flex-col sm:flex-col md:items-end gap-2 w-full md:w-auto">
                <div class="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-0">
                    <span class="text-sm text-gray-600 mb-1 sm:mb-0 sm:mr-2">Status:</span>
                    <div x-data="{
                ...
                <div class="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-0 mt-2 sm:mt-3">
                    <span class="text-sm text-gray-600 mb-1 sm:mb-0 sm:mr-2">Filter berdasarkan waktu:</span>
                    <div x-data="{
            ...
            <table class="min-w-full divide-y divide-gray-200 table-fixed">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th scope="col"
                            class="px-3 sm:px-6 py-2 sm:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                            <span class="hidden sm:inline">No. E-wallet</span>
                            <span class="sm:hidden">No.</span>
                        </th>

                        <th scope="col"
                            class="px-3 sm:px-6 py-2 sm:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                            <span class="hidden sm:inline">E-wallet</span>
                        ...
                        <th scope="col"
                            class="px-3 sm:px-6 py-2 sm:py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer"
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($riwayat as $item)
                        <tr wire:key="{{ $item->id }}" class="hover:bg-gray-50">
                            ...
                            @if (auth()->user()->role === 'admin')
                                <td class="px-3 sm:px-6 py-2 sm:py-4 whitespace-nowrap">
                                    <div class="text-xs sm:text-sm text-gray-900 truncate max-w-[120px] sm:max-w-[200px]">
                            ...
                            @endif
                            <td class="px-3 sm:px-6 py-2 sm:py-4 whitespace-nowrap">
                                <div class="text-xs sm:text-sm text-gray-900 truncate max-w-[80px] sm:max-w-full">
                                    {{ $item->no_telepon }}</div>
                            </td>
                            <td class="px-3 sm:px-6 py-2 sm:py-4 whitespace-nowrap">
                                <img src="{{ $item->metode_pembayaran === 'LinkAja' ? '/assets/linkaja.png' : '/assets//' . strtolower($item->metode_pembayaran) . '.svg' }}"
                                    alt="{{ $item->metode_pembayaran }}" class="h-6 sm:h-8 max-w-12 sm:max-w-16 object-contain">
                            </td>
                            <td class="px-3 sm:px-6 py-2 sm:py-4 whitespace-nowrap">
                                    ...
                                    @switch($item->status)
                                        @case('pending')
                                            <span wire:click="seeDetail({{ $item->id }})" @click="openModal = true"
                                                class="px-2 sm:px-3 py-1 text-xs rounded-full bg-yellow-100 text-yellow-800 font-medium 
                            hover:bg-yellow-200 cursor-pointer transition-colors duration-200 
                            hover:shadow-sm inline-flex items-center">
                                                <span>Pending</span>
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3 sm:h-3.5 sm:w-3.5 ml-1"
                                                    fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        ...
                                        @case('completed')
                                            <span wire:click="seeDetail({{ $item->id }})" @click="openModal = true"
                                                class="px-2 sm:px-3 py-1 text-xs rounded-full bg-green-100 text-green-800 font-medium 
                            ...
                            hover:shadow-sm inline-flex items-center">
                                        ...
                                        @case('rejected')
                                            <span wire:click="seeDetail({{ $item->id }})" @click="openModal = true"
                                                class="px-2 sm:px-3 py-1 text-xs rounded-full bg-red-100 text-red-800 font-medium 
                            ...
                            hover:shadow-sm inline-flex items-center">
                                        ...
                                        @default
                                            <span wire:click="seeDetail({{ $item->id }})" @click="openModal = true"
                                                class="px-2 sm:px-3 py-1 text-xs rounded-full bg-gray-100 text-gray-800 font-medium 
                            ...
                            <tr>
                                <td colspan="{{ auth()->user()?->role === 'admin' ? '6' : '5' }}"
                                    ...
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <p class="text-sm">Tidak ada data penarikan</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
        ...
        <div x-cloak x-show="openModal"
            ...
            <div class="flex justify-between items-center mb-4">
                <h2 class="text-lg sm:text-xl font-semibold text-gray-800">Detail Penarikan</h2>
            ...
            <div class="overflow-y-auto flex-1">
                <div wire:loading wire:target="seeDetail"
                    ...
                    <div class="flex flex-col items-center justify-center">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-10 w-10 sm:h-12 sm:w-12 text-emerald-500 mb-3 animate-spin"
                            fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                        </svg>
                        <p class="text-gray-600 text-base sm:text-lg font-medium">Memuat data penarikan...</p>
                    </div>
                </div>
                ...
                <div wire:loading.remove wire:target="seeDetail">
                    @if ($penarikan)
                            ...
                            <div class="flex flex-col md:flex-row justify-between">
                                ...
                                <div class="flex items-center mb-4 md:mb-0">
                                    ...
                                    <div class="ml-3 sm:ml-4">
                                        ...
                                        <p class="text-xs sm:text-sm text-gray-500">No. Telp: {{ $penarikan->nasabah->no_telepon }}
                                        </p>
                                        <p class="text-xs sm:text-sm font-medium text-emerald-600">Saldo: Rp
                                            {{ number_format($penarikan->nasabah->profile->saldo, 0, ',', '.') }}</p>
                                    </div>
                                </div>

                                <!-- Transaction details -->
                                <div class="flex flex-wrap justify-between gap-3 sm:gap-4 md:gap-6">
                                    <div>
                                        <p class="text-xs text-gray-500">Tanggal & Waktu</p>
                                        <p class="text-xs sm:text-sm font-medium">{{ $penarikan->created_at->format('H:i, d F Y') }}</p>
                                    </div>
                                    <div>
                        ...
                        <div class="bg-white rounded-lg shadow-sm border border-gray-100 p-3 sm:p-4">
                            <h3 class="text-sm sm:text-base font-medium text-gray-700 mb-3">Informasi Penarikan</h3>
                                ...
                                <div>
                                    ...
                                    <div class="flex items-center mt-1">
                                        <img src="{{ $penarikan->metode_pembayaran === 'LinkAja' ? '/assets/linkaja.png' : '/assets//' . strtolower($penarikan->metode_pembayaran) . '.svg' }}"
                                            alt="{{ $penarikan->metode_pembayaran }}" class="h-6 sm:h-8 w-auto mr-2">
                                        <p class="text-xs sm:text-sm font-medium">{{ $penarikan->metode_pembayaran }}</p>
                                    </div>
                                </div>
                                <div>
                                    <p class="text-xs text-gray-500">Nomor E-wallet</p>
                                    <p class="text-xs sm:text-sm font-medium mt-1">{{ $penarikan->no_telepon }}</p>
                                </div>
                            </div>
                        </div>

                        @if (auth()->user()->role === 'admin' && $penarikan->status === 'pending')
                            <div x-data="{ showRejectConfirm: false, showApproveConfirm: false }" class="mt-4 sm:mt-6">
                                <!-- Confirmation Messages -->
                                <div x-cloak x-show="showRejectConfirm"
                                    class="bg-red-50 p-3 sm:p-4 rounded-lg mb-4 border border-gray-300">
                                    <h3 class="text-sm sm:text-md font-semibold mb-2">Konfirmasi Penolakan</h3>
                                    <p class="mb-4 text-xs sm:text-sm text-gray-700">Apakah Anda yakin ingin menolak permintaan
                                    ...
                                    <div class="flex justify-end space-x-3">
                                        ...
                                        <button type="button"
                                            class="px-2 sm:px-3 py-1 text-xs sm:text-sm hover:cursor-pointer rounded bg-red-600 hover:bg-red-700 text-white font-semibold"
                                ...
                                <div x-cloak x-show="showApproveConfirm"
                                    class="bg-green-50 p-3 sm:p-4 rounded-lg mb-4 border border-gray-300">
                                    <h3 class="text-sm sm:text-md font-semibold mb-2">Konfirmasi Penyelesaian</h3>
                                    <p class="mb-4 text-xs sm:text-sm text-gray-700">Apakah Anda yakin ingin menyelesaikan penarikan
                                ...
                                <div x-show="!showRejectConfirm && !showApproveConfirm"
                                    ...
                                    <button type="button" @click="showRejectConfirm = true;"
                                        class="flex items-center justify-center px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 hover:cursor-pointer transition duration-200 text-sm sm:text-base font-medium">
                                        Tolak Penarikan
                                    </button>
                                    <button type="button" @click="showApproveConfirm = true;"
                                        class="flex items-center justify-center px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 hover:cursor-pointer transition duration-200 text-sm sm:text-base font-medium">
                    ...
                    @else
                        <div class="flex flex-col items-center justify-center py-8">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-10 w-10 sm:h-12 sm:w-12 text-gray-400 mb-3" fill="none"
                                viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M9.172 16.172a4 4 0 015.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                            </svg>
                            <p class="text-gray-500 text-base sm:text-lg">Data tidak ditemukan</p>
                        </div>
                    @endif
                </div>
            </div>
