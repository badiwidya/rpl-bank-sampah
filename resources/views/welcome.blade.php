<x-layouts.app title="Bank Sampah">
    ...
    <section id="berita"
        style="background-image: url('{{ asset('assets/bg-post-land-page.png') }}'); background-size: cover; background-position: center; background-repead: no-repeat;"
        class="h-screen flex flex-col justify-center items-center overflow-hidden">
        <h1 class="text-white text-2xl font-semibold mt-16">Berita Sampah Terkini</h1>
        <div class="flex-1 flex gap-8 p-8">
            @forelse ($posts as $post)
                <div
                    class="h-full flex flex-col p-4 w-64 bg-gray-100 {{ $loop->index === 1 ? '-translate-y-2.5 hover:-translate-y-3.5' : 'hover:-translate-y-1' }} rounded-lg transition-all duration-300">
                    <a href="{{ route('post.show', ['post' => $post->slug]) }}"
                        class="h-36 overflow-hidden rounded-md mb-2">
                        <img src="{{ Storage::url($post->images->first()->image_url) }}" alt="{{ $post->judul }}"
                            class="w-full h-full object-cover">
                    </a>
                    <a href="{{ route('post.show', ['post' => $post->slug]) }}"
                        class="font-semibold text-center hover:underline mb-2">{{ $post->judul }}</a>
                    <p class="text-gray-600 text-sm text-center">{!! Str::limit(strip_tags($post->konten), 150) !!}</p>
                </div>
            @empty
            @endforelse
        </div>
        <a href="{{ route('post.index') }}"
            class="flex uppercase p-3 mb-8 rounded-xl bg-gray-100 hover:bg-gray-200 hover:cursor-pointer transition duration-300">
            Lihat Semua
    ...
    <section class="h-screen bg-[#013021] flex flex-col justify-center items-center overflow-hidden">
        <div class="aspect-square h-16 w-16 overflow-hidden mb-4 mt-16">
            <img src="{{ asset('assets/hand-around-leaf.png') }}" alt="Hand around leaf"
                class="h-full w-full object-cover">
        </div>
        <div class="text-white w-[70%] text-center text-lg mb-4">
        ...
        <div class="flex-1 flex flex-col items-center p-8 mb-16 bg-white rounded-xl w-[90%]">
            <h2 class="font-semibold text-2xl">Bagaimana Cara Kerjanya</h2>
            <div class="flex justify-around items-center flex-1 w-full">
                <div class="flex flex-col items-center">
                    <div
                        class="flex justify-center items-center aspect-square rounded-full w-32 h-32 overflow-hidden hover:-translate-y-2 bg-emerald-100 transition-all duration-400">
                        <img src="{{ asset('assets/register.svg') }}" alt="Phone" class="w-16 h-16">
                    </div>
                    <p class="font-semibold mt-4">Daftar</p>
                    <p class="text-gray-600">"Bergabung untuk berpartisipasi."</p>
                </div>
                <div class="flex flex-col items-center">
                    <div
                        class="flex justify-center items-center aspect-square rounded-full w-32 h-32 overflow-hidden hover:-translate-y-2 bg-emerald-100 transition-all duration-400">
                        <img src="{{ asset('assets/collect.svg') }}" alt="Phone" class="w-16 h-16">
                    </div>
                    <p class="font-semibold mt-4">Kumpulkan</p>
                    <p class="text-gray-600">"Kumpulkan dan pisahkan sampah."</p>
                </div>
                <div class="flex flex-col items-center">
                    <div
                        class="flex justify-center items-center aspect-square rounded-full w-32 h-32 overflow-hidden hover:-translate-y-2 bg-emerald-100 transition-all duration-400">
                        <img src="{{ asset('assets/impact.svg') }}" alt="Phone" class="w-16 h-16">
                    </div>
                    <p class="font-semibold mt-4">Berdampak</p>
                    <p class="text-gray-600">"Selamatkan lingkungan bumi."</p>
                </div>
            </div>
        </div>
    </section>

    <section id="hubungi-kami" class="h-screen flex flex-col justify-center items-center overflow-hidden">
        <div class="h-[70%] bg-[#059669] w-[100%] flex flex-col justify-center items-center overflow-hidden gap-12">
            <h1 class="text-5xl text-white font-semibold">Siap gunakan layanan kami?</h1>
            <p class="text-center text-white w-[70%] text-2xl font-light">"Tukarkan sampah Anda di Bank Sampah, bantu
                lingkungan
                tetap bersih dan dapatkan manfaat berkelanjutan!"
            </p>
            <a href="{{ route('auth.login.choice') }}"
                class="py-3 px-8 bg-white rounded-full text-[#059669] hover:bg-gray-300 hover:cursor-pointer transition duration-300">Coba
                Sekarang</a>
        </div>
        <div class="h-[30%] bg-[#1F2937] w-[100%] flex flex-col">
            <div class="flex-1 flex text-3xl font-semibold text-white gap-4 justify-center items-center">
            ...
            <div class="flex justify-between p-2">
                <div class="flex gap-2 text-white text-xl items-center">
                ...
                <div class="flex gap-4 text-white text-xl items-center">
                    <svg xmlns="http://www.w3.org/2000/svg" class="size-8" viewBox="0 0 24 24" fill="currentColor">
                        <path
                ...
                <div class="flex gap-4 text-white text-xl items-center">
                    <svg xmlns="http://www.w3.org/2000/svg" class="size-8" viewBox="0 0 50 50" fill="currentColor">
                        <path
